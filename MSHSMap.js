    ...
    for(var roomname in sciRooms){
        (function (room, roomname) {
            room.attr(sciRoomStyle);
            room[0].addEventListener("mouseover", function() {
                room.animate(hoverStyle, animationSpeed);
                var node = document.getElementById("myPopup").innerHTML = roomname;
                myFunction();
            }, true);  
            room[0].addEventListener("mouseout", function() {
    ...
    a.addEventListener('submit',function(e) {
        e.preventDefault();
        var b = document.getElementById('textinput').value;
        b = b.toUpperCase();
        if(b.includes("A128"))
            {
        mathRooms[b].animate(hoverStyle, animationSpeed);
            }
        document.getElementById("myPopup").innerHTML = b;
        myFunction();
        inputs[2].disabled = false;
    });
            c.addEventListener('submit', function(f) {
            myFunction();
            f.preventDefault();
            var b = document.getElementById('textinput').value;
            b = b.toUpperCase();
            mathRooms[b].animate(mathRoomStyle, animationSpeed);
            a.reset();
